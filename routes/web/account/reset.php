...
Route::get(
    '/',
    [PasswordResetsController::class, 'index']
)->name(NamedRoute::GET_PASSWORD_RESET_INDEX);

// Sends an email with a reset link after validation
Route::post(
    '/',
    [PasswordResetsController::class, 'storeRequest']
)->name(NamedRoute::POST_PASSWORD_RESET_STORE);
...
Route::group(['prefix' => 'activate'], function () {
    ...
    Route::get(
        '/',
        [PasswordResetsController::class, 'activate']
    )->name(NamedRoute::GET_PASSWORD_RESET_TOKEN_VALIDATION_RESULT);

    // This route only validates the token, which then redirects to the index of this group
    Route::get(
        '/{token}/{email}',
        [PasswordResetsController::class, 'validateToken']
    )->name(NamedRoute::GET_PASSWORD_RESET_VALIDATE_TOKEN);

    Route::group(['prefix' => 'change-password'], function () {
        // This route will display the result of the password change
        Route::get(
            '/',
            [PasswordResetsController::class, 'changeStatus']
        )->name(NamedRoute::GET_PASSWORD_RESET_CHANGE_RESULT);

        // This route will display the password change form
        Route::get(
            '/{token}/{email}',
            [PasswordResetsController::class, 'reset']
        ...
        )->name(NamedRoute::POST_PASSWORD_RESET_CHANGE_STORE);
    });
});
